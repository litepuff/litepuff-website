...
function SummaryRow({ label, children, strong = false }) {
  return <div className={`flex items-center justify-between gap-4 ${strong ? 'text-[#243029]' : 'text-[#626964]'}`}><span>{label}</span><span className={strong ? 'font-semibold' : ''}>{children}</span></div>;
}
