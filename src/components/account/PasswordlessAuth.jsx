...
export default function PasswordlessAuth({ onComplete }) {
  ...
  return <div className="w-full">
    ...
    {step === 'otp' && <div className="text-center"><FiMessageCircle className="mx-auto text-[#C89B3C]" size={30} /><h2 className="mt-3 font-display text-2xl font-semibold">Check your {challenge?.provider === 'email' ? 'email' : 'WhatsApp'}</h2><p className="mt-2 text-sm text-[#68706B]">Enter the 6-digit code sent to {challenge?.destination || identifier}.</p><div className="mt-6 flex justify-center gap-2" onPaste={paste}>{digits.map((digit, index) => <input key={index} ref={(node) => { refs.current[index] = node; }} value={digit} inputMode="numeric" aria-label={`OTP digit ${index + 1}`} onChange={(e) => updateDigit(index, e.target.value)} onKeyDown={(e) => { if (e.key === 'Backspace' && !digits[index] && index) refs.current[index - 1]?.focus(); }} className="h-14 w-11 rounded-xl border border-[#DED8CC] text-center text-xl font-bold outline-none focus:border-[#1E4D3A] sm:w-12" />)}</div><button onClick={() => verify()} disabled={busy || digits.some((digit) => !digit)} className="mt-6 flex h-14 w-full items-center justify-center rounded-full bg-[#1E4D3A] font-bold text-white disabled:opacity-60">{busy ? <FiLoader className="animate-spin" /> : 'Verify & continue'}</button></div>}
