...
export default function WelcomeOffer() {
  ...
  return (
    <AnimatePresence initial={false}>
      {visible && (
        <motion.aside initial={{ opacity: 0, y: -42 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -42 }} className="fixed inset-x-0 top-0 z-[60] h-[42px] overflow-hidden border-b border-white/10 bg-[#1F5E3B] text-white" aria-label="Welcome offer">
          <div className="mx-auto flex h-full max-w-7xl items-center justify-center overflow-hidden px-12 text-center sm:px-16">
            <motion.p className="whitespace-nowrap text-[11px] font-medium tracking-[0.01em] sm:text-xs md:text-[13px]" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.45 }}><span aria-hidden="true">🎉 </span>Welcome Offer — Flat 10% OFF on your First Order <span className="mx-2 text-white/40">•</span> Use Code <strong className="text-[#F1D986]">LITEPUFF10</strong> <span className="mx-2 text-white/40">•</span> Free Shipping above ₹499</motion.p>
          </div>
