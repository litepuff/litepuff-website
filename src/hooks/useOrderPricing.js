...
export function useOrderPricing(items = [], options = {}) {
  ...
  return useMemo(() => ({ ...calculateOrderPricing({ items, couponDiscount: options.couponDiscount, freeShipping: options.freeShipping, firstOrderEligible: eligible }), firstOrderEligible: eligible, estimated: true }), [items, eligible, options.couponDiscount, options.freeShipping]);
}
