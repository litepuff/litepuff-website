...
export class GoogleSheetsConfig {
  ...
  validate() {
    const missing = Object.entries({ GOOGLE_SHEET_ID: this.spreadsheetId, GOOGLE_SERVICE_EMAIL: this.serviceAccountEmail, GOOGLE_PRIVATE_KEY: this.privateKey }).filter(([, value]) => !value).map(([key]) => key);
