...
export class WhatsAppOTPProvider {
  ...
  async send({ identifier, code, purpose }) {
    try { const result = await this.messaging.sendTemplate({ to: identifier, template: WHATSAPP_TEMPLATE_TYPES.AUTHENTICATION, variables: { code } }); logger.info('auth.otp.sent', { provider: this.name, purpose, deliveryStatus: result.status }); return { providerMessageId: result.messageId, status: result.status }; }
